<template>
    <div class="countdown">
        <div class="countdown_title">距离毕业还有</div>
        <div class="countdown_content">
            <div class="left_day">{{ left_day }}<span>天</span></div>
            <div class="left_hour">{{ left_hour }}<span>时</span></div>
            <div class="left_min">{{ left_min }}<span>分</span></div>
        </div>
        <div class="countdown_oneword">
            {{ saying }}
        </div>
    </div>
</template>
...
// import bgimg from '@/assets/countdown.jpg'
import moment from 'moment'
export default {
    data() {

        const S = DataConfig.getConfig();
        let lt = moment(S.examTime).diff(moment()) / 60000
        // moment.locale("zh-cn");
        // console.log('tdiff', lt);
        const left_day = parseInt(lt / (24 * 60));
        lt = lt % (24 * 60);
        const left_hour = parseInt(lt / 60);
        lt = lt % 60;
        const left_min = parseInt(lt);
        return {
            S: S,
...
.countdown {
    ...
    // background: linear-gradient(70deg, #042342, #04c196); 
    font-family: "Helvetica Neue 华文楷体";
    background: url('../assets/countdown.jpg') 100% 100%;
